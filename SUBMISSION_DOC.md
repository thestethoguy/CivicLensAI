...
The image is sent to **Gemini 1.5 Flash** operating as a "Professional Civil Engineering Auditor." The agent performs computer-vision-grade analysis and returns a fully structured JSON response:
...
Any citizen can click **"Verify Issue"** on any report. This triggers the second autonomous agent: the **Community Consensus Agent**, also powered by Gemini 1.5 Flash. This agent independently re-evaluates the issue and produces:
...
| Feature | Description |
|---|---|
| 🤖 **Smart Intake Agent** | Gemini 1.5 Flash Vision classifies the issue category, rates severity with engineering rationale, and auto-drafts a formal municipal email — all from a single photo |
...
|---|---|
| **Google AI Studio — Gemini 1.5 Flash** | **Intake Agent**: Multimodal computer vision analysis of civic issue photos. Produces structured JSON output (`category`, `severity`, `severity_rationale`, `clean_description`, `municipal_draft`) with `temperature=0.1` for deterministic, professional output |
| **Google AI Studio — Gemini 1.5 Flash** | **Community Consensus Agent**: Independent text-mode re-evaluation of reported issues. Generates `verification_score` (80–99%), `impact_analysis`, and `action_plan` as a second autonomous agent pass on the same issue |
